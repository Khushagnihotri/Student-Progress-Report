...
def init_db():
    ...
    if "class_name" not in existing_columns:
        cursor.execute("ALTER TABLE predictions ADD COLUMN class_name TEXT")
    if "roll_no" not in existing_columns:
        cursor.execute("ALTER TABLE predictions ADD COLUMN roll_no TEXT")

    # Keep only the latest record per student (class_name + roll_no) before adding uniqueness.
    cursor.execute(
        """
        ...
        WHERE id NOT IN (
            SELECT MAX(id)
            FROM predictions
            WHERE class_name IS NOT NULL AND roll_no IS NOT NULL
            GROUP BY TRIM(class_name), TRIM(roll_no)
        )
    ...
    cursor.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS idx_predictions_class_roll_unique
        ON predictions (class_name COLLATE NOCASE, roll_no COLLATE NOCASE)
        """
    )
...
def parse_student_meta(payload):
    class_name = str(payload.get("class_name", "")).strip()
    roll_no = str(payload.get("roll_no", "")).strip()

    if not class_name:
        raise ValueError("Missing required field: class_name")
    if not roll_no:
        raise ValueError("Missing required field: roll_no")
    return class_name, roll_no
...
def predict():
    try:
        ...
        class_name, roll_no = parse_student_meta(payload)
    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                ...
                (attendance, study, assign_score, lms, score, risk, ai_provider, ai_suggestion, class_name, roll_no)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(class_name, roll_no) DO UPDATE SET
...
def predictions_history():
    ...
    cursor.execute(
        """
        SELECT id, attendance, study, assign_score, lms, score, risk, ai_provider, ai_suggestion, class_name, roll_no, created_at
        FROM predictions
...
def student_search():
    ...
    if not class_name or not roll_no:
        return jsonify({"error": "class_name and roll_no are required."}), 400

    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
        SELECT id, class_name, roll_no, attendance, study, assign_score, lms, score, risk, ai_provider, ai_suggestion, created_at
        FROM predictions
        WHERE class_name = ? AND roll_no = ?
        ORDER BY created_at DESC
        LIMIT 1
        """,
        (class_name, roll_no),
...
@app.route("/class/risk-distribution", methods=["GET"])
def class_risk_distribution():
    class_name = request.args.get("class_name", "").strip()
    if not class_name:
    ...
    for row in rows:
        if row["risk"] in distribution:
            distribution[row["risk"]] = int(row["count"])

    return jsonify({"class_name": class_name, "distribution": distribution})
